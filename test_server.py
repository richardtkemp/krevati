...
class FakeSearcher:
    ...
    def search(self, term, n_results=5):
        self.last_query = term
        self.last_limit = n_results
        return self.results


def make_client(searcher, api_key='testkey'):
...
def test_query_is_read_from_the_request_body():
...
def test_response_is_a_json_list_of_results():
...
def test_missing_query_returns_400_and_does_not_call_searcher():
    fake = FakeSearcher()
    client = make_client(fake)
...
def test_omitted_limit_uses_a_default_and_succeeds():
    fake = FakeSearcher()
    client = make_client(fake)
...
def test_non_int_limit_returns_400():
    fake = FakeSearcher()
    client = make_client(fake)
...
def test_remote_with_correct_token_is_allowed():
    client = make_client(FakeSearcher(), api_key='secret')
    resp = client.post('/search', json={'query': 'x', 'limit': 5},
...
def test_pretty_print_of_no_results_is_empty_string():
    assert make_socketserver().pretty_print([]) == ''


def test_pretty_print_includes_score_path_header_and_snippet():
...
def test_pretty_print_truncates_snippet_to_200_chars():
